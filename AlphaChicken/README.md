...
```
AlphaChicken/
├── model.py         - Neural network architecture (ResNet + Policy/Value heads)
├── mcts.py          - Monte Carlo Tree Search with UCT
├── agent.py         - Main agent interface (game engine entry point)
├── train_pace.py    - Self-play training loop
└── best_model.pt    - Trained model weights (generated by training)
```
...
```
Input (8×8×14 board representation)
    ↓
[Conv Block] → [4× Residual Blocks]
    ↓                    ↓
...
|---------|-------------|------|
| 0 | My chicken position | One-hot (8×8) |
| 1 | Enemy chicken position | One-hot (8×8) |
...
1. **Policy Head**: 12-dimensional log-softmax
   - Maps to 4 directions × 3 move types
...
- **Temperature control**: Exploration (τ=1) → Exploitation (τ=0)
...
1. If no time → return first valid move
2. If move invalid → choose from valid moves
3. If no children in tree → random valid move

---

## 🏋️ Training (`train_pace.py`)
...
├── AlphaChicken/
│   └── agent.py
└── (game module imported from parent)
```
