...
class Evaluator:
    ...
    def heuristic(self, game_board: board.Board) -> float:
        ...
        Returns:
            Heuristic score from current player's perspective (higher is better)
        """
        score = 0.0
        ...
        if len(my_moves) == 0:
            # Being blocked is catastrophic (-5 eggs for opponent)
            score -= 5000.0
        ...
        if len(opp_moves) == 0:
            # Blocking opponent is great (+5 eggs for us)
            score += 5000.0
        ...
        # Egg difference is the most important factor
        ...
        # Effective penalty is 8 egg swing
        score -= risk_here * 800.0
        score -= max_risk_nearby * 200.0
