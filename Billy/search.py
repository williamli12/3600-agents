...
================================
The game engine provides Board objects where:
  - board.chicken_player: the "current player" from this board's perspective
  - board.chicken_enemy: the opponent from this board's perspective
  - board.forecast_move(dir, move_type): applies a move for chicken_player and returns
    a NEW board. The perspective does NOT automatically switch - chicken_player remains
    the same before and after forecast_move.
  - board.reverse_perspective(): swaps chicken_player ↔ chicken_enemy.
...
Our search maintains the following invariant:
  ...
  - Evaluation (heuristic/value net) ALWAYS evaluates from the perspective of 
    board.chicken_player. Since we ensure chicken_player = OUR agent in both max
    and min contexts (via careful perspective management), maximizing in max nodes
    and minimizing in min nodes produces correct adversarial search.
"""
...
class SearchEngine:
    ...
    The search is fully adversarial with correct perspective handling at all levels.
    """
    ...
    def choose_move(
        ...
        while depth <= effective_max_depth:
    ...
    def _choose_move_greedy(
        ...
        """
        Simple fallback: simulate each move once, evaluate fully, pick best.
        ...
        for move in valid_moves:
            ...
            if child_board is not None:
                val = self.evaluator.evaluate(child_board)
                if val > best_val:
                    best_val = val
                    best_move = move
    ...
    def _max_value(
        ...
        Args:
            game_board: Current board state (chicken_player = our agent)
    ...
    def _min_value(
        ...
        Minimizing the evaluation score (which is from our perspective) corresponds
        to the opponent picking moves that are worst for us.
        ...
        Returns:
            Best value for minimizing player (worst for us)
        """
        self.nodes_searched += 1
        ...
        # Terminal test (evaluate from our perspective, as invariant requires)
        if self._terminal_test(game_board, depth, time_left):
            return self.evaluator.evaluate(game_board)
    ...
    def _simulate_move(
        ...
        - forecast_move() applies the move for chicken_player and returns a NEW board.
        - The perspective does NOT automatically switch after forecast_move.
        - If is_opponent=False: we're simulating our own move, so we don't need to
          reverse (chicken_player stays as our agent).
        - If is_opponent=True: we're simulating opponent's move from a board where
          opponent is chicken_player. After forecast_move, opponent is still 
          chicken_player, so we reverse_perspective() to restore our agent as 
          chicken_player for the next _max_value call.
        ...
        # forecast_move applies move for chicken_player; does NOT auto-switch perspective
        child_board = game_board.forecast_move(direction, move_type)
        
        if child_board is None:
            return None
        
        # If opponent move: reverse perspective so chicken_player = our agent again
        if is_opponent:
